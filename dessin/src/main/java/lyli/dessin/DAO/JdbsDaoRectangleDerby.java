...
+ "VALUES (?, ?, ?, ?, ?)");
		...
		prepare.setInt(QUATRE, obj.getsideTop());
		prepare.setInt(CINQ, obj.getsideLeft());
...
+ "WHERE NameForme = ?");
			...
			prepare.setInt(TROIS, obj.getsideTop());
			prepare.setInt(QUATRE, obj.getsideLeft());
			prepare.setString(CINQ, obj.getNameForme());
			int result = prepare.executeUpdate();
			assert result == 1;
		}
	} catch (SQLException e) {
